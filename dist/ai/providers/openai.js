...
class OpenAIProvider extends base_1.AIProvider {
    constructor(apiKey, model) {
        ...
        if (!key) {
            throw new Error('OPENAI_API_KEY not provided. Set the OPENAI_API_KEY environment variable or pass it in the config.');
        }
        this.apiKey = key;
...
${JSON.stringify(scanResults, null, 2)}`;
        const content = await this.chatCompletion(REVIEW_SYSTEM_PROMPT, userPrompt, REVIEW_TEMPERATURE, REVIEW_MAX_TOKENS);
...
${fileContent}
\`\`\``;
        const content = await this.chatCompletion(FIX_SYSTEM_PROMPT, userPrompt, FIX_TEMPERATURE, FIX_MAX_TOKENS);
        const parsed = this.parseJSON(content);
        return {
